...
function unisWithUni(array) {
  const unisWithUni = [];
  // guess you could filter here as well
  array.forEach(grad => {
    grad.university.includes("Uni") && unisWithUni.push(grad);
  });
  return unisWithUni;
}
...
function populationTotal(array) {
  const populationTotal = array.reduce((acc, cur) => {
